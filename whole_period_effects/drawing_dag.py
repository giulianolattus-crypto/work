...
def plot_causal_networks_two_seasons(network_data_list, positions_list=None, 
                              titles=None,row_labels=['SON', 'DJF'], figsize=(32, 16)):
    """
    Plots 4 causal directed networks in a 2x2 grid layout.

    Parameters:
    - network_data_list: list of 4 dicts. Each dict contains edge tuples (source, target) as keys,
                         and a 'mean' value as the attribute, e.g. {("ENSO", "SPV"): 0.3, ...} (not really mean value anymore, but should still work?)
    - positions_list: optional list of 4 dicts with node positions, e.g. {"ENSO": (0,2), ...}
    - titles: optional list of 4 subplot titles
    - figsize: size of the entire figure
    """
    rows=int(len(network_data_list)/2)
    cols=int(len(network_data_list)/2)
    fig, axes = plt.subplots(rows, cols, figsize=figsize)
    axes = axes.flatten()
    for idx, edge_dict in enumerate(network_data_list):
        ...
        for u, v, d in G.edges(data=True):
            ...
            # Color only if the edge points into T or Precip
            if v == 'T_Andes_SON' or v=='T_LP_SON' or v=='T_Andes_DJF' or v=='T_LP_DJF':
                color = 'red' if weight >= 0 else 'blue'
            elif v == 'Precip_Andes_SON' or v=='Precip_LP_SON' or v=='Precip_Andes_DJF' or v=='Precip_LP_DJF':
                color = 'green' if weight >= 0 else '#b8860b'  # ochre
            else:
                color = 'gray'  # default color for other edges
            edge_colors.append(color)
    ...
    fig.suptitle("Cross-seasonal Causal Network with Whole Period Regression Coefficient Values", fontsize=60,fontweight='heavy', y=1.02)
    plt.tight_layout()
    fig.savefig('DAG_with_reg_vals_SON_DJF.pdf', bbox_inches='tight')
    plt.show()
